...
public abstract class CheeseDb extends RoomDatabase{
    ...
    private static volatile CheeseDb sInstance;

    public static CheeseDb getInstance(){
        if(sInstance==null){
            synchronized (CheeseDb.class){
                if(sInstance==null){
                    sInstance= Room.databaseBuilder(LibraryTest.sLibraryTest,
                            CheeseDb.class,"CheeseDatabase")
    ...
    private static void fillInDb(){
        AppExecutors.getInstance().getIoExecutors().execute(
                ()->{
                    ...
                    for(String name: Arrays.asList(CHEESE_DATA)){
                        cheeses.add(new Cheese(name));
                    }
                    getInstance().cheeseDao().insert(cheeses);
                }
        );
    }

    private static final String[] CHEESE_DATA={"Abbaye de Belloc", "Abbaye du Mont des Cats", "Abertam", "Abondance", "Ackawi",
